...
class interstatePriceService {
  ...
  getAllInterstateAddress = (pageSize, page, skip) => {
    return new Promise(async (resolve, reject) => {
      try {
        let data = await InterstateAddress.find({})
          .skip(skip)
          .limit(pageSize);
      ...
      try {
        ...
        if (!checkExist) {
          return reject({
            code: 400,
            msg:
              "Specified interstate address does not exists",
          });
        }
        const address = await entryInstance.getGooglePlace(options.address)
        const Geo = address[0].geometry.location;
        const data = Object.assign(options, Geo)
        let updateAddress = await InterstateAddress.findByIdAndUpdate({ _id: id }, data);
